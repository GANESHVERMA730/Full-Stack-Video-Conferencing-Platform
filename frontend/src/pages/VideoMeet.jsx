...
const createUnmirroredCameraStream = async (cameraStream) => {
  ...
  const correctedVideoStream = canvas.captureStream(30);

  cameraStream.getAudioTracks().forEach((track) => {
    correctedVideoStream.addTrack(track);
  });

  return correctedVideoStream;
};
...
export default function VideoMeetComponent() {
  ...
  const getPermissions = async () => {
    try {
      ...
      if (videoPermission || audioPermission) {
        ...
        if (userMediaStream) {
          const correctedStream =
            await createUnmirroredCameraStream(userMediaStream);

          window.rawCameraStream = userMediaStream;
          window.cameraStream = correctedStream;
          window.localStream = correctedStream;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (window.rawCameraStream) {
        window.rawCameraStream.getTracks().forEach((track) => {
          track.stop();
        });
      }
    };
    ...
    if (fromId !== socketIdRef.current) {
      ...
      if (signal.sdp) {
        peer
              ...
              peer
                ...
                .then((description) => {
                  peer
                    ...
                    .catch((e) =>
                      console.error(
                        "Error setting local answer SDP:",
                        e
                      )
                    );
                })
          ...
          .catch((e) =>
            console.error(
              "Error setting remote description SDP:",
              e
            )
          );
      }
      ...
      socketRef.current.on(
        ...
        (id, clients) => {
          ...
          if (id === socketIdRef.current) {
            for (
              let id2 in connectionsRef.current
            ) {
              if (
  ...
  const getDisplayMediaSuccess =
    useCallback((screenStream) => {
      ...
      for (
        let id in connectionsRef.current
      ) {
        if (id === socketIdRef.current) {
          continue;
        }
        ...
        if (videoSender) {
          videoSender
            ...
            .catch((error) => {
              console.error(
                "Error replacing camera with screen:",
                error
              );
            });
        }
